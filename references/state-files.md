...
- `acceptance` (tool-written for delivery Nodes): the deterministic acceptance snapshot. `phase` is one of `awaiting_acceptance_design`, `acceptance_designer_running`, `awaiting_acceptance_review`, `acceptance_reviewer_running`, `acceptance_revision_required`, `awaiting_executor`, `executor_running`, `repair_required`, `awaiting_regression`, `awaiting_auditor`, `auditor_running`, `repair_plan_required`, `awaiting_repair`, or `accepted`; `attempt` is a non-negative counter; `outcome` is bounded state-machine data. An unenrolled Node begins at `awaiting_acceptance_design`. Rejection or preparation drift in an enrolled lifecycle parks the same Node at `acceptance_revision_required` with `main_acceptance_decision`; an explicit designer dispatch may revise that Node, but no leaf is selected automatically. A running dispatch contains only an opaque `id`, its role, and the bounded fingerprints required by that role. Final repair handoff may contain a `repair_node_id`; accepted state contains bounded audit fingerprint metadata. Never hand-edit this object.
...
- The native parent reads `next-action`; `dispatch --role acceptance_designer|acceptance_reviewer|executor|auditor` obtains the runtime platform, checks prerequisites and unique active work, validates the phase/contract/criterion requirements, marks the Node active when execution begins, and records one opaque attempt. Direct `start`, `regress`, `check`, and `complete` are rejected for delivery Nodes.
- While `executor_running`, the fresh code-only executor implements the selected closure. Declared ownership is a planned focus rather than a filesystem boundary; necessary adjacent implementation changes are reported to the native main. The executor cannot mutate Plan state, edit frozen tests, run acceptance, or mark its own result.
- The correlated `executor-exited` event runs the focused contract automatically against unchanged declared paths. Failure enters `repair_required` with `main_repair_decision`; success enters `awaiting_auditor` with a fresh receipt. Command output is discarded.
- `dispatch --role auditor` binds the independent audit to that receipt. `audit-failed` clears proof and returns implementation to repair; `audit-passed` checks every mapped criterion and auto completes the Node.
...
| `dispatch <node-id> [root] --role acceptance_designer\|acceptance_reviewer\|executor\|auditor` | create or reuse one correlated native-agent dispatch when that role is required |
| `advance <node-id> [root] --event <event> [--dispatch-id <id>] [--repair-node <id>]` | consume `acceptance-designer-exited`, `acceptance-approved`, `acceptance-rejected`, `executor-exited`, `regression-requested`, `audit-passed`, `audit-failed`, `repair-registered`, or `repair-completed`; guarded events run regression, route repair, and auto complete |
