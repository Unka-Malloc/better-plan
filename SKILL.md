...
- Keep traceability from start to finish. Default to Plan-local labels such as `REQ-001`; every label must begin with `REQ` and use hyphen-delimited alphanumeric segments. Never put a Plan prefix before `REQ`. Record the same canonical labels in requirements, implementation Nodes, validation criteria, and each Node's `requirements` field. Final validation must prove the requirements defined at the beginning, not a different target discovered later. Use `scripts/manifest_tool.py check-labels` to keep document labels and Node labels consistent whenever either side changes.
...
7. `scripts/manifest_tool.py validate` also compares every Plan and Node status against the file's git HEAD version and rejects changes that no transition path allows, such as `completed` back to `in_progress`. Run it after every state file edit; use `transition <current> <target>` to check a single edge.
   ...
   - `check-labels [workspace] [--plan <selector>]` cross-checks requirement labels between plan documents and Node `requirements`; noncanonical document or Node labels are errors. Run it after editing either side.
