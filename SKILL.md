...
- Worker: `references/worker.md`. Its brief supplies the complete Task, owned requirements, authorized
  goal/scope/success/risk boundary, global architecture, and all resolved decisions. v3 has no reliable
  Task applicability link for ledger decisions; do not guess a filter. Read other original Plan
  context only as needed via `plan_path`.
...
- Optional multi-plan execution: `references/coordination.md`; token, wait, and parallel-efficiency
  audits: `skills/efficiency-inspector/SKILL.md`.
...
decisions, and repository facts to exactly one fresh-context Designer in its exclusive session. Forward the `assignment`
returned by `open-designer-session` unchanged. Do not pre-design Tasks or enlist the Reviewer early.
...
exclusive resources. Keep all dependencies inside Tasks so the whole frontier is parallel-safe.
...
The Designer chooses standard/complex difficulty from coupling, unknowns, tradeoffs, failure impact,
and verification burden, never risk tags alone. Workload (`light|medium|heavy`) describes execution
volume independently of tier, without clock estimates. Tasks also declare verification
(`code|visual|hybrid`) and `worker: general|frontend`. Record only useful design dimensions and cover
every owned requirement and output with executable acceptance; implementation recommendations leave
Workers room for judgment inside binding guarantees.
...
state writes. For a Codex frontend Task, the native main checks the local `frontend-worker` before
implementation: a valid configured role must be dispatched; an absent optional role falls back to
the Task's exact standard/complex tier. Host batching never changes contracts or permits an
...
Optional read-only aids: `scripts/task_shape.py <Plan.json> --task TASK-001` summarizes the Node DAG;
`scripts/wait_hint.py --workload heavy --elapsed 1800 --progress 35 --history 4800` suggests a wait
window. Neither tool is mandatory.

### 5. Accept Tasks and handle missing prerequisites

After final Worker callbacks, run `accept-task` concurrently for the entire awaiting frontier. Python
locks only the snapshot and result commit; commands inside each Task retain their declared order.
...
For explicitly requested multi-plan execution, use `scripts/coordination_tool.py` with its reference.
Source-native acceptance owns Task progress; external requirements gate only consumers. Configure
the host-neutral adapter and record exact-unit user authorization before `tick` or `watch`. The
private journal stores transport correlation; v3 Nodes remain inside their Task.
